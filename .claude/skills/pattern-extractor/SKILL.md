...
5. **Cross-Agent Knowledge**: Generate insights for sharing with other agents
...
**JSON** (machine-readable, for sharing):
```json
{
...
0 6 * * * cd <project-root> && /pattern-extractor --last 1 --update-file memory/technical-patterns.md
```
...
```javascript
// Extract patterns and broadcast to other agents
const patterns = await exec('pattern-extractor --last 7 --format json');
...
- Optional: LLM API (for pattern summarization)
...
**Why it works**: APIs need proper boundary in Content-Type for multipart uploads
...
- Always use teams (too expensive) — rejected
- Never suggest teams (misses opportunities) — rejected
- Analyze and suggest (balanced approach) — chosen
...
- Cross-agent patterns need standardized format
...
4. **Cross-repo patterns**: Find patterns across multiple projects
