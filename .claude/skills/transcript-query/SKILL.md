...
1. **Vector embeddings**: Use vector search for semantic queries
2. **Automatic summarization**: Use an LLM API to summarize sessions
...
- `--cli name` — Filter by specific CLI (claude|codex|cursor|gemini)
...
- Codex transcript format: `.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`
- Cursor transcript format: `.cursor/projects/*/agent-transcripts/*.txt`
- Gemini CLI transcript format: `.gemini/tmp/<sha256>/chats/session-*.json`
