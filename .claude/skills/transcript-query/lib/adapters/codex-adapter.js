      ...
      switch (event.type) {
        ...
        case 'response_item':
          if (event.payload?.type === 'message') {
            ...
            const contentBlocks = Array.isArray(event.payload?.content) ? event.payload.content : [];
            ...
            if (role === 'assistant') {
              for (const item of contentBlocks) {
                ...
                if (item.arguments) {
                  try {
                    parameters = typeof item.arguments === 'string'
            ...
            if (event.payload?.arguments) {
              try {
                parameters = typeof event.payload.arguments === 'string'
                  ? JSON.parse(event.payload.arguments)
                  : event.payload.arguments;
              } catch {
                parameters = { raw: event.payload.arguments };
              }
            }
            data.toolUses.push({
        ...
        case 'turn_context':
          ...
          if (event.payload?.cwd && !data.cwd) data.cwd = event.payload.cwd;
          break;
      }
    }
