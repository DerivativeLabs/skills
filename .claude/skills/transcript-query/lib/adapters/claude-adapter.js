    ...
    try {
      ...
      for (const f of files) {
        ...
        try {
          ...
          // Top-level .jsonl — main session transcript
          if (f.endsWith('.jsonl')) {
            transcripts.push({
              cli: this.cli,
          ...
          // Session directory — recurse for subagent transcripts
          if (stats.isDirectory()) {
            ...
            try {
              ...
              for (const sf of subFiles) {
                ...
                const subPath = path.join(subagentsDir, sf);
                try {
                  const subStats = await fs.stat(subPath);
                  transcripts.push({
                    cli: this.cli,
                    sessionId: `${f}/subagents/${sf.replace('.jsonl', '')}`,
                    path: subPath,
